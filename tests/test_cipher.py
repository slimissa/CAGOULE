"""
test_cipher.py — Tests cipher/decipher CAGOULE v3.1.0
...
# ══════════════════════════════════════════════════════════════════════
#  Tests encrypt_bulk / decrypt_bulk — v2.4.0 P2
# ══════════════════════════════════════════════════════════════════════

class TestEncryptBulk:
    """Tests pour encrypt_bulk / decrypt_bulk (P2 v2.4.0)."""

    @pytest.fixture(autouse=True)
    def _imports(self):
        from cagoule.cipher import encrypt_bulk, decrypt_bulk
        from cagoule.decipher import decrypt
        self.encrypt_bulk = encrypt_bulk
        self.decrypt_bulk = decrypt_bulk
        self.decrypt = decrypt

    def test_roundtrip_basic(self):
        """Round-trip : 3 messages de tailles variées."""
        msgs = [b"alpha", b"beta" * 20, b"gamma" * 100]
        cts = self.encrypt_bulk(msgs, b"pwd", fast_mode=True)
        assert len(cts) == 3
        pts = self.decrypt_bulk(cts, b"pwd", fast_mode=True)
        assert pts == msgs

    def test_empty_list(self):
        """Liste vide → liste vide."""
        assert self.encrypt_bulk([], b"pwd") == []
        assert self.decrypt_bulk([], b"pwd") == []

    def test_single_message(self):
        """Liste de taille 1."""
        cts = self.encrypt_bulk([b"solo"], b"pwd", fast_mode=True)
        pts = self.decrypt_bulk(cts, b"pwd", fast_mode=True)
        assert pts == [b"solo"]

    def test_coherent_with_individual(self):
        """encrypt_bulk(msgs) déchiffrable message par message via decrypt()."""
        msgs = [b"msg" + bytes([i]) * 16 for i in range(8)]
        cts = self.encrypt_bulk(msgs, b"testpass", fast_mode=True)
        for i, ct in enumerate(cts):
            pt = self.decrypt(ct, b"testpass", fast_mode=True)
            assert pt == msgs[i], f"message {i} incorrect"

    def test_shared_salt(self):
        """Tous les ciphertexts d'un encrypt_bulk partagent le même sel."""
        msgs = [b"a", b"bb", b"ccc"]
        cts = self.encrypt_bulk(msgs, b"pwd", fast_mode=True)
        salts = [ct[5:37] for ct in cts]
        assert len(set(salts)) == 1, "Les sels doivent être identiques"

    def test_different_nonces(self):
        """Même sel → nonces distincts (évite la réutilisation de nonce)."""
        msgs = [b"same" * 4] * 5
        cts = self.encrypt_bulk(msgs, b"pwd", fast_mode=True)
        nonces = [ct[37:49] for ct in cts]
        assert len(set(nonces)) == 5, "Les nonces doivent être distincts"

    def test_wrong_password(self):
        """Mauvais mot de passe -> échec journalisé, résultat None (pas d'exception).

        CORRECTIF (audit round 2) : decrypt_bulk() ne fait plus abort sur le
        premier échec (voir cipher.py). Un mot de passe incorrect affecte
        tout le batch de la même façon qu'un ciphertext isolé corrompu --
        chaque élément concerné devient None dans la liste de résultats,
        loggé via _log.warning, sans exception au niveau de l'appel. C'est
        le même contrat que decrypt_bulk_ctr() (decipher_ctr.py) pour le
        même scénario -- vérifié empiriquement avant ce correctif de test :
        decrypt_bulk_ctr(cts, wrong_password) retournait déjà [None], pas
        d'exception. Ce test alignait cipher.py sur l'ancien contrat
        fail-fast ; il documente maintenant le contrat réellement voulu.
        """
        cts = self.encrypt_bulk([b"secret"], b"correct", fast_mode=True)
        results = self.decrypt_bulk(cts, b"wrong", fast_mode=True)
        assert results == [None]

    def test_string_password(self):
        """Password str accepté."""
        cts = self.encrypt_bulk([b"hello"], "password", fast_mode=True)
        pts = self.decrypt_bulk(cts, "password", fast_mode=True)
        assert pts == [b"hello"]

    def test_large_batch(self):
        """Batch de 50 messages — cohérence garantie."""
        msgs = [f"message_{i}".encode() * 10 for i in range(50)]
        cts = self.encrypt_bulk(msgs, b"bigpwd", fast_mode=True)
        assert len(cts) == 50
        pts = self.decrypt_bulk(cts, b"bigpwd", fast_mode=True)
        assert pts == msgs

    def test_order_preserved(self):
        """L'ordre des résultats est préservé."""
        msgs = [bytes([i]) * 32 for i in range(20)]
        cts = self.encrypt_bulk(msgs, b"order", fast_mode=True)
        pts = self.decrypt_bulk(cts, b"order", fast_mode=True)
        assert pts == msgs

    def test_str_messages(self):
        """Messages str acceptés."""
        cts = self.encrypt_bulk(["hello", "world"], b"pwd", fast_mode=True)
        pts = self.decrypt_bulk(cts, b"pwd", fast_mode=True)
        assert pts == [b"hello", b"world"]
