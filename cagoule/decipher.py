"""
decipher.py — Déchiffrement CAGOULE v3.1.0
...
class CagouleAuthError(CagouleError):
    ...
    def __repr__(self) -> str:
        return (f"CagouleAuthError(reason={self.reason!r}, "
            f"ct_size={self.ct_size}, backend={self.backend!r}, hint={self.hint!r})")
...
def _backend_str() -> str:
    return "C (libcagoule.so v3.0.0)" if CAGOULE_C_AVAILABLE else "Python pur (fallback)"
...
def _detect_bad_field(data: bytes) -> str:
    ...
    if data[4:5] != VERSION:
        # CORRECTIF (audit round 2) : decipher.py::decrypt() est le
        # décodeur CBC (0x01) uniquement -- exposé publiquement comme
        # cagoule.decrypt_cbc(). "attendu 0x01" est donc correct dans ce
        # contexte, mais ne l'explique pas : quelqu'un appelant
        # decrypt_cbc() sur un ciphertext 0x02/0x03 par erreur voyait un
        # message qui semblait dire "ce n'est pas un CAGOULE valide",
        # alors que 0x02/0x03 SONT valides -- juste pas pour CETTE
        # fonction. Message explicite plutôt que de renommer/déplacer
        # la fonction (rétrocompatibilité de l'API publique).
        return (
            f"version (reçu 0x{data[4]:02x}, attendu 0x01 -- cette fonction "
            f"est le décodeur CBC uniquement (decrypt_cbc()). Pour 0x02 "
            f"(CTR) utiliser decrypt_ctr(), pour 0x03 (RAW) utiliser "
            f"decrypt_ctr_raw(), ou cagoule.decrypt() pour un dispatch "
            f"automatique par VERSION)"
        )
    if len(data) < 37:
...
# ── §5. Helper pour decrypt_bulk ───────────────────────────────────────

def _parse_cgl1_salt(data: bytes) -> bytes:
    """Extrait le sel (bytes[5:37]) d'un ciphertext CGL1 sans décrypter."""
    if len(data) < 37:
        raise ValueError(f"Ciphertext trop court pour extraire le sel : {len(data)}")
    magic = data[0:4]
    if magic != b'CGL1':
