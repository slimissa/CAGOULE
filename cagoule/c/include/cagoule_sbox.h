/**
 * cagoule_sbox.h — S-box Feistel 32-bit CAGOULE v3.1.0
 *
...
/**
 * Chiffrement Feistel AVX2 sur n éléments.
 * Traite 4 lanes en parallèle ; queue traitée en scalaire.
 * Compilé uniquement si __AVX2__ est défini ; fallback scalaire sinon.
 * Appelé depuis cagoule_cipher.c après dispatch runtime.
 */
...
/**
 * Déchiffrement Feistel AVX2 sur n éléments.
 */
...
/**
 * Retourne 1 si le backend AVX2 S-box est actif, 0 sinon.
 * Exposé à Python via ctypes pour backend_info.
 */
int cagoule_sbox_backend_is_avx2(void);

/**
 * Retourne TOUJOURS 0 -- il n'existe aucune implémentation NEON du
 * S-box dans cette base de code (voir cagoule_sbox_avx2.c pour le
 * commentaire complet). Présent uniquement pour la symétrie de l'API
 * de détection attendue par get_backend_info_v310() côté Python.
 */
