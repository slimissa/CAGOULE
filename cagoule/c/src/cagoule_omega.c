/**
 * cagoule_omega.c — Pilier Ω : ζ(2n) → Round Keys — CAGOULE v3.1.0
 *
 ...
 *  §5. HKDF-SHA256
 *
 *  CORRECTIF (audit round 2, consolidation) : implémentation locale
 *  (hkdf_extract/hkdf_expand/hkdf_sha256, ~50 lignes dupliquant
 *  cagoule_kdf.c) supprimée. Remplacée par un fin wrapper autour de
 *  cagoule_kdf_hkdf() (cagoule_kdf.c), qui a reçu le correctif H3
 *  (nettoyage des buffers intermédiaires -- absent de la version locale
 *  ici, jamais corrigé puisqu'elle n'avait pas été identifiée comme
 *  duplication avant cet audit).
 *
 *  Vérifié AVANT ce remplacement, pas après : comparaison directe des
 *  deux implémentations sur 20000 essais aléatoires avec le pattern
 *  d'appel réel de ce fichier (ikm de longueur variable, info=23 octets
 *  fixes, out_len=32 fixe) -- 0 écart. Les deux utilisent le même sel
 *  zéro RFC 5869 §2.2 (salt=None -> HashLen zéros), même construction
 *  HKDF standard. L'unique site d'appel ci-dessous reste dans le domaine
 *  testé : out_len=32 (une seule limite : ≤32 dans l'implémentation
 *  locale disparue -- la version partagée supporte le multi-bloc, donc
 *  plus permissive, pas moins) et info_len=23 (bien en-deçà des limites
 *  des deux versions, 255 locale vs 200 partagée).
 *
...
{
    if (cagoule_kdf_hkdf(ikm, ikm_len, info, info_len, out, out_len)
        != CAGOULE_KDF_OK)
        return CAGOULE_OMEGA_ERR_HKDF;
    ...
    uint64_t      *keys_out)
{
    ...
    for (int k = 1; k <= num_keys; k++) {
        ...
        if (ret != CAGOULE_OMEGA_OK) {
            /* CORRECTIF (P0-1, unifié P2-8) : key_material (ak_seed ||
             * salt || n_bytes) et rk_bytes (sortie HKDF brute
             * pré-réduction) contiennent du matériel sensible et
             * doivent être nettoyés même sur ce chemin d'erreur --
             * sinon ils persistent sur la pile après le retour.
             * OPENSSL_cleanse() : même convention que cagoule_kdf.c,
             * cagoule_params.c, cagoule_stream.c, cagoule_matrix.c,
             * cagoule_api.c -- résistant à l'élimination de code mort
             * par l'optimiseur, contrairement à une boucle volatile
             * manuelle. */
            OPENSSL_cleanse(rk_bytes, SHA256_LEN);
            OPENSSL_cleanse(key_material, km_len);
            return ret;
        }
 ...
 * ══════════════════════════════════════════════════════════════════════════ */
 ...
 /* Note: In the v2.3.0 CBC pipeline, round key addition is vectorized
 * via addmod64x4/addmod64 in cagoule_cipher.c. This function is retained
 * for standalone use and backward compatibility with omega.py. */
...
/* NOTE (audit round 2, consolidation) : cette fonction n'a PAS été fusionnée
 * avec le nettoyage OPENSSL_cleanse() de cagoule_params.c/cagoule_stream.c
 * -- ce n'est pas le même type de fonction. Celles-là zéroïsent des buffers
 * précis passés en paramètre ; celle-ci est un point d'entrée d'API
 * publique pour un futur nettoyage d'état global/statique, actuellement
 * un no-op car aucun état de ce type n'existe encore. Rien à consolider. */
